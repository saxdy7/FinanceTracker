...
export default function ModernSidebar({ pathname: propPathname, onLogout, onClose }) {
  ...
  return (
    <aside className="w-64 h-screen bg-white border-r border-gray-100 flex flex-col fixed left-0 top-0 z-30 shadow-sm">
      ...
      <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
        ...
        {onClose && (
          <button onClick={onClose} className="lg:hidden p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition">
            <X className="w-4 h-4" />
          </button>
        )}
      </div>
